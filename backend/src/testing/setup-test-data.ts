...
import nodeForgePackage from "node-forge";

const { random, util } = nodeForgePackage;
...
const getAllRecords = async (table: string) => {
  if (
    !table.toLocaleLowerCase().includes("aai") &&
    !table.toLocaleLowerCase().includes("local")
  )
    throw Error(
      "I'm not prepared to nuke all the data from a table unless it contains the string 'AAI' and 'Local' in its name",
    );
...
/**
 * A statically generated RSA key - we are totally happy with this being checked in public source - it
...
/**
 * Sets up the test data in dynamo for a refreshed local run
 */
export async function setupTestData(): Promise<void> {
  ...
  const id = staticTestData
    ? "static"
    : cryptoRandomString({ length: 16, characters: safeCharacterSet });
  const jwksKey = staticTestData ? staticAustralia : await makeRsaJoseWithPrivateFromSeed();
  const clientId = staticTestData
    ? "client"
    : cryptoRandomString({ length: 16, characters: safeCharacterSet });
  const clientSecret = staticTestData
    ? "secret"
    : cryptoRandomString({ length: 16, characters: safeCharacterSet });
